...
| F10  | The `DB_URL` refusal exists only on the env-**file** path. A caller writing their own `bin/` can pass `env: { DB_URL: '…' }` and reach the identical plaintext Lambda variable with no guard. ADR 0005 explicitly invites that.                            | **Closed in slice 2, on the second attempt.** The first fix refused `DB_URL` alone, exact case, and this register wrongly recorded it closed — a review then synthesized `db_url`, `DB_GRANT_ROLE_URL`, `POSTGRES_PASSWORD` and sixteen others straight into the template. `assertConfigurableEnv` now reads the same three lists and the same key-shape rule as the file path, covers `props.web.env`, and `env-boundary-parity.test.ts` fails if the two boundaries diverge again. |
...
### Slice 2's own review, 2026-09-06

Green-field byte-identity was verified past the oracle — raw `cdk.out` diffed against a
clean checkout at slice 1: 112 resources both sides, identical logical-ID sets,
identical ordering, no `DependsOn` change, differences confined to container-image
digests. `executeAfter` is member- and order-identical on both green-field paths.

| From | Finding                                                                                                                                                                                                                                 | Disposition                                                                                                                                                                                    |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F-A  | `assertDatabaseSelection` refused one key where the env file refused twenty. See F10 above.                                                                                                                                             | **Fixed in slice 2.**                                                                                                                                                                          |
| F-B  | "Routes the API through CloudFront" was satisfied by the _web_ function's origin — mutation-confirmed to pass with the API origin removed entirely.                                                                                     | **Fixed in slice 2.** The test follows `/graphql*` to its target origin to the function URL. Both fixes were mutation-tested.                                                                  |
| F-C  | On the BYO path nothing orders the migration after VPC egress. Green-field was masked by Aurora's ~10-minute create always beating the NAT gateway; removing the cluster from `executeAfter` removed that accident. Affects topology D. | **Slice 3**, where `hasVpc` makes the conditional natural. Must be conditional on `!ownsDatabase`, or it adds `DependsOn` entries to the green-field template and breaks the governing oracle. |
| F-D  | `props.env: { DB_URL: '' }` was not refused and reached the functions as `DB_URL: ""`, under a test claiming equivalence with the file path.                                                                                            | **Fixed in slice 2.** No blank carve-out on the props boundary.                                                                                                                                |
| F-E  | Four doc comments the slice made false.                                                                                                                                                                                                 | **Fixed** across slices 2 and 3.                                                                                                                                                               |
| F-F  | `props.web.env` was a second unguarded plaintext surface.                                                                                                                                                                               | **Fixed in slice 2.**                                                                                                                                                                          |
| F-G  | Topology C is unreachable, and `bin/grant.ts` still passes `database: {}` unconditionally, so no reference-app topology can reach refusal F.                                                                                            | Slice 3 and **slice 4**. When slice 4 adds `-c dbUrlSecretArn` it must make `database` conditional, or every BYO deploy trips the F refusal.                                                   |

### Gate 4's independent pass, 2026-09-09

The blocking pass over the **assembled** path, by a reviewer who authored none of the
slices. It found the F10 shape a third time, which is the finding worth carrying out of
this story: a refusal that exists on one configuration boundary and not on the parallel
one, recorded as closed because it was closed _somewhere_.

**This register records the mutation that now fails, not the fact that a fix landed.**
F10 was ticked once on a fix that covered one key of twenty; C-1 below was ticked on a
fix that covered three lists of four. A disposition that cannot name a failing test is
the same claim that has now been wrong twice.

| From | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Disposition                                                                                                                                                                                                     |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C-1  | **Critical.** `REFUSED_AS_ENV` omitted `RESOLVER_SECRET_KEYS`, so `AUTH_MFA_SECRET_ENCRYPTION_KEY` and `GITHUB_CLIENT_SECRET` — routed to the platform secret by the env file — synthesized as **plaintext Lambda environment variables** through `props.env` and `props.web.env`. Not inert: the resolver reads `payload[name] ?? process.env[name]`, so the plaintext works. The MFA key derives the AES-256 key over every stored TOTP seed. The exclusion's stated reason named `secrets` as the alternative one clause before concluding there was none. | **Fixed.** Dropping `...RESOLVER_SECRET_KEYS` from the list fails **6 tests**; deleting the `props.env` call site fails 2.                                                                                      |
| C-1b | The parity oracle was blind here. Its resolver-key case asserted `fileRefuses(key) === false` and **never called `propsRefuses`**, on exactly the two keys it named.                                                                                                                                                                                                                                                                                                                                                                                          | **Fixed.** The case now asserts all three boundaries: the file routes, `props.env` refuses naming `secrets`, and `props.secrets` accepts.                                                                       |
| H-1  | **High.** `props.secrets` was a **fourth** boundary with no key validation of any kind. `ORIGIN_VERIFY_SECRET` — refused by name on both others — was accepted into the `generateStringKey` of the same secret. It is the only control in front of an `AuthType: NONE` function URL, and `platform-secret.ts` states it must never pass through the template.                                                                                                                                                                                                 | **Fixed** by `assertConfigurableSecrets`. Removing the generated-key branch fails 2 tests; deleting the call site fails 2.                                                                                      |
| M-1  | A `secrets.DB_URL` silently overrode the validated `databaseUrl` — `PlatformSecret` spreads `extraEnv` _after_ the composed value — reaching the same field having passed neither the scheme check nor the quote check. An empty string reintroduced F4 by another door.                                                                                                                                                                                                                                                                                      | **Fixed** with H-1: `STACK_COMPOSED_KEYS` is refused on `secrets`. Removing that branch fails 2 tests.                                                                                                          |
| M-2  | The `web.env` guard (F-F) had **no coverage**: deleting `assertConfigurableEnv(props.web?.env, 'web.env')` left **430/430 green**. The only test naming it asserted the string label passed to the validator, not that anything called it. The F-B pattern.                                                                                                                                                                                                                                                                                                   | **Fixed.** `props-boundary-leak.test.ts` synthesizes a real platform, so it fails on a deleted **call site**. The same mutation now fails 2 tests.                                                              |
| M-3  | The guide told adopters to set `DB_GRANT_ROLE_URL`, which the env file refuses, `props.env` refuses, and `migrate` **silently ignores** — no error, no effect, and the operator holds a superuser URL with nowhere to put it. F2's disposition had required the guide say the grant is run manually, _not_ configured; it said both.                                                                                                                                                                                                                          | **Fixed.** The guide now says to run the grant yourself and points at the statements.                                                                                                                           |
| M-4  | **F5 is unmeasured and was carried nowhere** — neither in the gate 4 checklist nor in § Follow-ons — so merging would lose it.                                                                                                                                                                                                                                                                                                                                                                                                                                | **Accepted as an open risk**, recorded as follow-on 5 below. Needs an account; the synth-time half was confirmed safe (`JSON.stringify` escapes a literal quote), the deploy-time textual substitution was not. |
| L-1  | `.env.example` and `props.ts` stated the out-of-band rotation workaround as fact. The measurements file says it is reasoned, not observed, and must not be presented otherwise. The guide carried the caveat; these two did not.                                                                                                                                                                                                                                                                                                                              | **Fixed** in both.                                                                                                                                                                                              |
| L-2  | `migrate` took the same `-c dbUrlSecretArn` as `cdk deploy` with none of its constraints, and passed no `--region` — so a default region holding a same-named secret migrates the wrong database, quietly.                                                                                                                                                                                                                                                                                                                                                    | **Fixed.** Pins the CLI to the ARN's region and rejects a malformed ARN; a bare name stays supported, since `DatabaseSecretName` is published as a name.                                                        |

**Verified clean by execution**, and worth recording because these are the claims the
story rests on: no logical-ID drift (`synth:check` green, and the green-field snapshot
diff empty across the whole story); no credential material in the committed BYO
snapshots; `DB_URL` in no Lambda environment on any topology; **no IAM widening for the
VPC-less path** — the set of statements present in either BYO template and absent from
green-field is empty, and topology C is strictly narrower; topology B's ingress
identity-scoped to `DatabaseClients` on 5432 with no CIDR; 18 spellings of `DB_URL` all
fail closed; no shell injection in the migrate shell-out. § Risks row 6 was verified
rather than accepted: the SSRF guard references no VPC, NAT, egress address or route,
so topology C narrows what a function can reach without removing anything the guard
relies on.
...
feat/byo-database..main` = 0), so the trunk is a clean fast-forward from
  ...
  - [x] Independent security pass over the assembled BYO path — run 2026-09-09 by a
        reviewer who authored none of the slices. **It blocked**: one Critical, one
...
- [x] `git worktree remove ../grant-pr384` (stale from #384), plus the four this
      ...
      **true as of 2026-09-09, not when this was first ticked**: `../grant-byo-slice5`
...
| 5   | **F5 — a `"` inside the _referenced_ secret** (gate 4, M-4). Whether CloudFormation merely breaks, or the quote can close `DB_URL` and open another key inside the platform secret. | Needs a deploy: a literal is refused at synth, but a referenced secret's contents are not visible there, and the substitution is textual at deploy time. Accepted as an open risk rather than lost at merge — it is a privilege boundary wherever whoever writes the connection string is not whoever may set `AUTH_MFA_SECRET_ENCRYPTION_KEY`. |
