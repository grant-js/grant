...
| F6  | `project-sync` now runs on the jobs function, under Lambda's 15-minute ceiling. ADR 0002 routes imports past that ceiling to a container runtime, and phase C was to wire it. Slice 6 measured a real import at **208 s / 283 entities** — room under 15 min, and **not** the 28,880-entity measurement ADR 0002 still owes.        | **Closed by the aws-followups-closeout story** (slices 12a–13, #433): measured; Fargate hatch is opt-in; visibility left alone. Merged to `main` as #445.                                                                                                                                                                                               | aws-followups-closeout #433                                                                                                                                                          |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| 1   | **Make credential keys resolver-backed** (gate 4 F-C, option 2). `classifyConfig` now _refuses_ ~15 credential-shaped keys because Lambda env vars are template plaintext and the adapters read them from `process.env`. | **Closed** by aws-followups-closeout slices 7–8 (#407, #408).                  |
| 2   | **Presigned-PUT uploads.** Lifts the ~6 MB Lambda payload cap; the nginx gateway allows 100 MB.                                                                                                                          | **Closed** by aws-followups-closeout slices 9–11 + F-3 (#427–#430, #440–#442). |
| 3   | **Bring-your-own PostgreSQL, end to end** (F13). Omitting `database` also omits the API function, which reads `DB_URL` from the platform secret the stack creates beside its own cluster.                                | **Closed** by the byo-database story (#394).                                   |
| 4   | **An alarm on the origin-verify warn log.** The security review sustained the public-Function-URL risk _with_ this as the named compensating control, and it is not wired.                                               | **Closed** by aws-followups-closeout slice 3 (#403).                           |
| 5   | **`project-sync` at ADR 0002 scale, and the Fargate escape hatch** (program blocker 3).                                                                                                                                  | **Closed** by aws-followups-closeout slices 12a–13 (#433).                     |
| 6   | **RDS Proxy on by default, and RDS IAM auth** (F14). Proxy is off because it forfeits Aurora auto-pause; IAM auth was a phase B leftover never built.                                                                    | **Closed as a decision** by aws-followups-closeout slice 14 (#435).            |
| 7   | **OpenNext.** The web app runs as a Next standalone server on Lambda; the acceptance criterion is marked `[~]`.                                                                                                          | **Closed as a decision** by aws-followups-closeout slice 15 (#434).            |
| 8   | **A test asserting middleware order** (gate 4 F-D). Origin verification must precede the rate limiter; it does today, and nothing asserts it.                                                                            | **Closed** by aws-followups-closeout slice 1 (#400).                           |
| 9   | **Scope `ses:SendEmail` to the sending identity** (F-E). Currently `Resource: "*"`, so a compromised function could send from any verified identity in the account.                                                      | **Closed** by aws-followups-closeout slice 2 (#401).                           |
