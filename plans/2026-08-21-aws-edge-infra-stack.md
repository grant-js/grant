...
| F15 | **Slice 7 did not record `cdk destroy`.** Verification model required teardown after every deployed slice. The 2026-09-05 scratch stack may still be running (~$93/month).                                                                                                                                                          | **Blocking for gate 4.** Confirm the stack is gone (or destroy it) and append the residue table to the measurements file before opening the integration PR.                                                                            | Principal            |
...
1. **Blocking — scratch account (F15).** Slice 7's deploy is not recorded as
   destroyed. Confirm `cdk destroy --all` (or that the account is already at
   baseline) and append the residue table before opening the PR. An adopter's
   first real action after evaluating is teardown; shipping without that last
   measurement repeats the exact gap the verification model was written to close.
...
- [ ] Scratch account torn down; final `cdk destroy` recorded **(F15 — not evidenced for slice 7)**
