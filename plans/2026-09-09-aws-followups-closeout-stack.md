...
| 17b (F-3)           | **open, stacked on 17a**       | same plan                                                                      |
...
| 17b   | `cursor/organization-direct-upload-7203`   | 17a   | F-3  | Org `request`/`confirm`; delete web `toDataUrl`              | Frontend + Backend | light             |      |
...
- [ ] **Gate 4: story → `main` deep review.** Blocking items, known in advance:
  ...
  - [x] The scratch account measured in **both** regions after slice 16 — **done, and the
        ...
        RDS, ACM, secrets, ECS, SQS, and the zone back to 20 records), but log groups
        (135 / 16) and 2.40 GB of bootstrap ECR images are not, and cannot be by
        `cdk destroy`. Recorded as F7 and follow-on 16 rather than ticked as "zero";
        gate 4 should accept the measurement or reject the wording, not both.
  - [x] `main` merged into the trunk before the integration PR opens — done 2026-09-13
        (`a09f1d07`), trunk 0 behind. `main` had moved by five commits; #431 brought a
        third column with F-3's defect, which is why slice 17 exists. Six conflicts, one
        of which git did not report: the merge compiled only after adapting #431's org
        dialog to slice 11's upload contract.
...
- [x] Scratch account torn down after slice 16 — 2026-09-13, 24.7 min; both regions at
      baseline for stacks, RDS, ACM, secrets (checked with `--include-planned-deletion`),
      ECS and SQS; F-1's ACM CNAME removed by hand and the zone back to 20 records; F7
      recorded (135 / 16, of which **one per region is not Grant's** — see the
      measurements file, which corrects cycle 1's "+7 per cycle" model) and a new
      residue class counted as follow-on 16
