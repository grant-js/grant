...
- **Status**: merged-to-main (2026-08-08, [#225](https://github.com/grant-js/grant/pull/225))
...
| A3  | ~~Re-run the pass-1 lenses and update `api.md` with resolved counts.~~ **Done.** Close-out table added in [#222](https://github.com/grant-js/grant/pull/222); re-run again at gate 4, which moved one row (`cache-handler.ts` 655 → 675, slice 10 added hardening back) and caught a bad check of my own — a grep reporting 97 domain-error imports was matching mentions, not imports | Already on the cleanup checklist; listed here so it is not missed                                                                                                                                                               |
...
| B3  | CDM repository ports for `ProjectImportRepository` (15 public methods) and `ProjectExportRepository` (18) — **needs its own story brief**, not a slice | [Slice 2a](#2a--cdm-repository-ports--dropped-from-this-story-2026-08-07-ale-heredia), dropped from this story. ~12 return types are declared inside the repositories; the ports require migrating them into `@grantjs/core` first, which changes what the domain package owns |

### C — Code, ordered by risk × value

| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Size      |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| C1  | Zod input validation for the services that have none. **7 of the 12, not 12** — `email`, `file-storage`, `me`, `notifications`, `project-sync-job`, `signing-keys`, `user-mfa` are referenced from `handlers/`; `event-relay`, `grant`, `notification-delivery`, `project-export` and `webhook-delivery` are reached only from jobs and workers, so they take no request input. `signing-keys`, `user-mfa` and `me` are the three that want a security bar | 7 files   |
...
- [x] Gate 3: Stack PRs merged into trunk — 1, 2, 3, 4, 4a, 5, 6, 7, 8, 9. Slice 2a dropped (see above).
  - Slice 5 security-full: author-reviewed at merge. Independent pass 2026-08-07 returned **BLOCK**; remediated by **slice 10**, merged as [#223](https://github.com/grant-js/grant/pull/223) before gate 4.
  - Slice 8 security-full: reviewed at merge.
- [x] Gate 4: Story → `main` deep review complete — [#225](https://github.com/grant-js/grant/pull/225), merged 2026-08-08.
  - Verified on the final trunk: format, lint (apps/api 0 problems), `dead-code:api`, type-check, build, **961 unit**, **189 e2e** (2 skipped) via the root wrapper.
  - CodeQL raised 4 × `js/missing-rate-limiting` on `crud-router.ts`, dismissed as false positive against existing repo precedent. Rate limiting is global (`server.ts:102`, mounted before the routers; `SKIP_PATHS` is `/health` only) and the three pre-slice-7 route files had no route-level limiting either — the alerts are new only because slice 7 moved the handlers. 25 alerts of the same rule remain untriaged on `main`, pre-existing and out of scope.

## Cleanup

- [x] `git worktree remove` — not needed; the story ran in the main checkout, no worktree was created
- [x] Local slice branches deleted — 15, each confirmed merged on GitHub first. Note for next time: `git log main..<branch>` reports every squash-merged branch as unmerged, because the original commits never enter `main`'s history. The authoritative check is the PR's merge state, not commit ancestry
- [x] Stack plan status → `merged-to-main`
- [x] Re-run the pass-1 lenses and update [`api.md`](../docs/contributing/code-quality/api.md) with resolved counts — see A3

## Handed to pass 2

Recorded in the [rubric's passes table](../docs/contributing/code-quality/README.md#passes) so it does not live only here.

**The guardrails this story built stop at `apps/api`.** Slice 3 scoped them deliberately — 11 rule blocks in `eslint.config.mjs` target `apps/api/**`, and `dead-code:api` is `knip --workspace apps/api` — because a full-repo run would have surfaced violations in `apps/web` and `packages/` that this story had no mandate to fix. Widening them is the first slice of each unit's own pass, not an afterthought: a rule that runs before the audit counts exactly what the auditor would otherwise estimate by hand.
