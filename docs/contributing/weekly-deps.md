...
3. `pnpm install`, then typecheck, lint, and `pnpm test`. Run e2e (`pnpm test:e2e`) when the lockfile or runtime deps changed and the e2e stack is up. CI uses **Node 24** and runs `pnpm run build` before `pnpm run synth:check`. Reproduce that locally (`pnpm --filter grant-docs build` then `pnpm --filter grant-aws-deploy synth:check`) and commit `deploy/aws/cdk.snapshot/` if it is dirty — do not push until the check is green on a clean tree.
