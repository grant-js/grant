...
export const permissionTags = pgTable(
  ...
  (table) => [
    ...
    index('permission_tags_deleted_at_idx').on(table.deletedAt),
  ]
);
...
export const permissionTagAuditLogs = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    permissionTagId: uuid('permission_tag_id').references(() => permissionTags.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => [
    ...
    index('permission_tag_audit_logs_scope_tenant_idx').on(t.scopeTenant),
  ]
);
