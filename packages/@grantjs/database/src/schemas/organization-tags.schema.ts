...
export const organizationTags = pgTable(
  ...
  (table) => [
    ...
    index('organization_tags_deleted_at_idx').on(table.deletedAt),
...
export const organizationTagAuditLogs = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organizationTagId: uuid('organization_tag_id').references(() => organizationTags.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => [
    ...
    index('organization_tag_audit_logs_scope_tenant_idx').on(t.scopeTenant),
  ]
);
