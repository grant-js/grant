...
function parseNotificationScopeIds(scope: NotificationEventScope): ParsedScopeIds {
...
/**
 * Dashboard path for a notification deep-link. Mirrors the web helper; never
 * returns personal account project admin URLs.
 */
export function buildNotificationDashboardPath(params: {
...
export function buildNotificationInboxPath(): string {
  return '/dashboard/notifications';
}

export function buildNotificationPreferencesPath(): string {
  return '/dashboard/settings/notifications';
}

export function absoluteFrontendUrl(path: string, locale: SupportedLocale = defaultLocale): string {
  const normalized = path.startsWith('/') ? path : `/${path}`;
  return `${config.security.frontendUrl}/${locale}${normalized}`;
}
