...
// Direct Upload Schemas — the presigned path. The base64 schemas above stay.
export const requestMyUserPictureUploadUrlRequestSchema = z.object({
  filename: z.string().min(1, 'errors.validation.filenameRequired').openapi({
    description: 'Original filename with extension',
    example: 'profile.jpg',
  }),
...
export const requestMyUserPictureUploadUrlResponseSchema = createSuccessResponseSchema(
  z.object({
    url: z.string().openapi({
      description:
        'URL to PUT the bytes to. Absolute for object stores; resolve a relative URL against the API origin.',
      example: '/storage/users/123/picture.jpg?exp=1757600000&len=204800&ct=image%2Fjpeg&sig=...',
    }),
    ...
    expiresAt: z.string().openapi({
      ...
      example: '2026-09-11T09:05:00.000Z',
    }),
  }),
  'Upload URL issued'
);

export const confirmMyUserPictureUploadRequestSchema = z.object({
  filename: z.string().min(1, 'errors.validation.filenameRequired').openapi({
    description:
      'The same filename the URL was requested with. The storage path is re-derived from the caller identity, not taken from the client, so this only selects the extension.',
    example: 'profile.jpg',
  }),
});
