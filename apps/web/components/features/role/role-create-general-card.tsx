...
export function RoleCreateGeneralCard() {
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <PrimaryTagSelector
            control={form.control}
            name="primaryTagId"
            label={tRoles('form.primaryTag')}
            items={tags}
            loading={tagsLoading && tagIds.length > 0}
            loadingText={tRoles('form.tagsLoading')}
            emptyText={tRoles('form.noTagsAvailable')}
            alwaysVisible
            selectTagsFirstPlaceholder={tRoles('form.selectTagsFirstForPrimaryTag')}
          />
        </div>
      </div>
    </FeatureModuleCard>
