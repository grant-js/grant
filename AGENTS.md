...
- **"Unused export" means something different in each of the trio, and the difference is measurable.** What is semver-public is what the built `.d.ts` entry points re-export, not what `src` declares — so read the artifact, not the source:
  - **`client`** — 19 source exports, all 19 public (plus `Scope`/`Tenant` re-exported from `@grantjs/schema`, so its contract is _wider_ than what it defines). Nothing is safely deletable.
  - **`server`** — 37 source exports, 31 public, **6 internal**. Only those 6 are encapsulation candidates.
  - **`cli` has no module API at all.** Its `exports` map declares only `"."`, and the built `dist/index.d.ts` is `#!/usr/bin/env node` + `export {};`. `src/config/index.ts` re-exports 17 symbols that are **not** public despite looking it — the barrel is consumed internally, by `commands/`. **`cli`'s contract is its command surface** (`grant start`, `grant config`, `grant generate-types`, `grant version`) and its config-file format. Treat a change to either as breaking; treat its TypeScript exports as internal.
...
- **The published trio is exempt for the same structural reason, one step further out.** `@grantjs/server` declares its own `GrantServerError` hierarchy (shadowing four of core's names), `client` throws 3 raw `Error`s and `cli` 18. Importing `@grantjs/core` to comply would add a **private, unpublished** workspace package to the npm dependency graph of packages consumers install. Complying is not possible, so the exemption is recorded here rather than worked around in the code. Pass 7, `docs/contributing/code-quality/published-packages.md` § Tier 3.1.
